...
for parIndex = 1:2
    ...
    % % PlotHistory(x,t,[Z(:,[iVe iCK]) Se],[],[idx(:, [1 2]) [2*nx+1:3*nx]']);  
    % % PlotHistory(x,t,[Z(:,[iVe iVi iRa iCK]) Se Si Saa],[],[idx(:, [1 2]) [2*nx+1:3*nx]']); 
    % 
    % 
    % PlotHistory(x, t, all, [], idx);
    ...
    if parIndex == 1
        ...
        parValues = linspace(parIn, parFin, nOfPar);  % example list of parameter values
    ...
    elseif parIndex == 2
        ...
        error('Something went wrong! Check parIndex values! Stopping execution.');
    ...
    for i = 1:length(parValues)    
        ...
        if parIndex == 1
            ...
            modelp.gamma_an = parValues(i);
        
        elseif parIndex == 2
        
            modelp.gamma_aa = parValues(i);
